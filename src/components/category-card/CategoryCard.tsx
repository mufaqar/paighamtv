...
const CategoryCard = ({ item }: any) => {
  return (
    <div className='bg-red-300 relative group overflow-hidden'>
      <Link href={`/category/${item.slug}`} ><Image src={item?.categoryInfo?.catImage?.mediaItemUrl || DefaultImage} alt="image" width={700} height={700} className='w-full object-cover h-[420px] transition-all duration-200 ease-in-out group-hover:scale-105' />
